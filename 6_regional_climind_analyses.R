...
suppressMessages( pacman::p_load(config, profvis, tidyverse, giscoR, lwgeom, sf, tmap, grid) )
...
NAO_positive_dates = as.character( NAO[NAO$binary_thres==TRUE,1] )
NAO_negative_dates = as.character( NAO[NAO$binary_thres!=TRUE,1] )
...
for (r in 1:n_regions){
  ...
  # attributions$dates = as.character(all_dates); setindex(attributions, dates)
  # start = Sys.time()
  ...
  # attr_pos_phase = attributions[NAO_positive_dates, on="dates"]
  # attr_neg_phase = attributions[NAO_negative_dates, on="dates"]
  # end = Sys.time()
  # print( end-start )
  # print( "phases date filter complete" )
  ...
  # end = Sys.time()
  # print( end-start )
  # print( "AF mean calculation complete" )
